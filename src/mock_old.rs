...
use crate::{
    ...
    GraphIndexManager, GraphQueryExecutor, GraphStore, GraphTraversal, GraphTransaction, 
    MultiGraphManager,
};
#[cfg(feature = "mock")]
...
impl MultiGraphManager for MockGraphStore {
    async fn create_graph(&self, graph_info: GraphInfo) -> TylResult<GraphInfo> {
        ...
        if graphs.contains_key(&graph_info.id) {
            return Err(TylError::validation("graph_id", format!("Graph '{}' already exists", graph_info.id)));
        }
    ...
    async fn delete_graph(&self, graph_id: &str) -> TylResult<()> {
        ...
        transactions.retain(|_, tx| {
            !tx.metadata.get("graph_id").map(|v| v.as_str().unwrap_or("")).eq(&Some(graph_id))
        });
...
impl GraphStore for MockGraphStore {
    ...
    async fn delete_node(&self, graph_id: &str, id: &str) -> TylResult<()> {
        ...
        graph.relationships.retain(|_, rel| rel.from_node_id != id && rel.to_node_id != id);

        Ok(())
    }

    async fn create_relationship(&self, graph_id: &str, mut relationship: GraphRelationship) -> TylResult<String> {
        self.ensure_graph_exists(graph_id).await?;
